...
def get_site_errors(
    ...
    query = db.query(SEOError).filter(SEOError.site_id == site_id)
    if severity:
        query = query.filter(SEOError.severity == severity)
    
    return query.order_by(SEOError.created_at.desc()).limit(limit).all()
